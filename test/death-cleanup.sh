...
owner=
assert_equal '#[push-default]#[default] #[reverse]F#[default]#[default]#[pop-default]' "$(render)" 'dead running derives an unread virtual failure'
...
assert_equal '#[push-default]#[default]dead-other:#[reverse]F#[default] #[default]#[pop-default]' "$(TMUX="$server_tmux" "$root/scripts/render-other-sessions" "$session")" 'other-session rendering exposes the same virtual failure'
...
assert_equal '#[push-default]#[default] F#[default]#[pop-default]' "$(render)" 'acknowledged virtual failure remains visible without unread emphasis'
...
assert_equal '#[push-default]#[default] #[reverse]F#[default]#[default]#[pop-default]' "$(render)" 'dead waiting derives the same deterministic virtual failure'
...
assert_equal '#[push-default]#[default] #[reverse]C#[default]#[default]#[pop-default]' "$(render)" 'dead completed preserves its recorded terminal state and generation'
tmux_test set-option -s "@tmux-agents-status-ack-$pane" "$completed_generation"
assert_equal '#[push-default]#[default] C#[default]#[pop-default]' "$(render)" 'acknowledged dead completed remains visible without unread emphasis'

failed="v2|$incarnation|pid:99999999|-|failed|$failed_generation|-|-"
tmux_test set-option -s "@tmux-agents-status-state-$pane" "$failed"
tmux_test set-option -su "@tmux-agents-status-ack-$pane"
assert_equal '#[push-default]#[default] #[reverse]F#[default]#[default]#[pop-default]' "$(render)" 'dead failed preserves its recorded terminal state and generation'

tmux_test set-option -s "@tmux-agents-status-state-$pane" "v2|$incarnation|pid:$$|-|completed|$completed_generation|-|-"
tmux_test set-option -s "@tmux-agents-status-ack-$pane" "$completed_generation"
assert_equal '#[push-default]#[default] C#[default]#[pop-default]' "$(render)" 'acknowledgement preserves a live terminal state'

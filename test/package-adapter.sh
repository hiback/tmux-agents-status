...
workspace=$1
package=$2
entrypoint=${3:-tmux-agents-status.ts}
...
printf '%s\n' "package/package.json" "package/$entrypoint" |
...
adapter=$(tar -xOf "$tarball" "package/$entrypoint")
...
printf '%s\n' "$manifest" | grep -F "\"./$entrypoint\"" >/dev/null || fail "$package resolves only the adapter entrypoint"
